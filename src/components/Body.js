...
const fetchData = async () =>{
  const apiData= await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=18.61610&lng=73.72860&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING")
  ...
  return <Shimmer></Shimmer>
    ...
    return (
      <div className="body">
        ...
        <input className="search-input" type="text" value={searchText} onChange={(event)=>{
        ...
        <button className="filter-btn "
          onClick={()=>{
            const filteredData =restaurantData.filter((res) => res.info.avgRating > 4)
            setrestaurantData(filteredData);
